    ...
    particles.forEach((particle, i) => {
      particles.slice(i + 1).forEach(otherParticle => {
        ...
        if (distance < 100) {
          ...
          ctx.strokeStyle = `hsla(45, 100%, 72%, ${0.1 * (1 - distance / 100)})`;
          ctx.lineWidth = 1;
          ctx.stroke();
        }
      });
...
function initializeEasterEggs() {
  ...
  const now = new Date();
  if ((now.getHours() >= 23 || now.getHours() <= 6) && !easterEggsFound.has('nightowl')) {
...
function initializeVisitorCounter() {
  ...
  if (!sessionVisited) {
    ...
    let recentVisits = JSON.parse(localStorage.getItem('portfolioRecentVisits') || '[]');
  ...
  if (visitorCount) {
    ...
    animateVisitorCounter(visitorCount, Math.max(currentCount - 15, 0), currentCount, 2500);
  }
  ...
  const messages = [
    ...
    `Hello there! You're our ${currentCount}${getOrdinalSuffix(currentCount)} visitor`,
  ...
  setInterval(() => {
    if (Math.random() < 0.1) { // 10% chance every 30 seconds
      ...
      localStorage.setItem('portfolioVisitorCount', currentCount);
      if (visitorCount) {
        animateVisitorCounter(visitorCount, currentCount - increment, currentCount, 1000);
      }
    }
